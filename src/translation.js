export async function translate(text, settings, fetcher = fetch, language = 'ja') {
  if (!['ja', 'en'].includes(language)) throw new Error('不支持的翻译语言');
  ...
  if (settings.provider === 'microsoft') {
    ...
    response = await fetcher(`https://api.cognitive.microsofttranslator.com/translate?api-version=3.0&from=${language}&to=zh-Hans`, {
      method: 'POST', signal,
  ...
  url.search = new URLSearchParams({ q: text, langpair: `${language}|zh-CN` }).toString();
