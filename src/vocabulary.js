...
function render() {
  ...
  $('#review-count').textContent = `${words.filter(w => isDue(w) && w.meaning?.trim()).length} 词待复习`;
  const query = $('#search').value.trim().toLocaleLowerCase();
  const language = $('#word-language').value;
  let visible = words.filter(w => (language === 'all' || w.language === language) && (filter === 'all' || w.mastered === (filter === 'mastered')) && [w.surface, w.base, w.reading, w.meaning, w.romaji, w.note].some(t => t?.toLocaleLowerCase().includes(query)));
...
function wordCard(word) {
  const article = el('article', `word-card ${word.language === 'en' ? 'english-card' : ''}`);
  const top = el('div', 'word-card-top');
  top.append(el('span', `study-tag ${word.mastered ? 'mastered' : ''}`, word.mastered ? '✓ 已掌握' : '• 学习中'));
  top.append(el('span', 'language-tag', word.language === 'en' ? 'EN' : 'JA'));
  const audio = el('button', 'audio-button', '▷'); audio.title = `朗读 ${word.surface}`; audio.setAttribute('aria-label', `朗读 ${word.surface}`); audio.onclick = () => speak(word.surface, word.language).catch(e => notify(e.message, true)); top.append(audio);
  const open = el('button', 'word-open'); open.onclick = () => showDetail(word.id);
  open.append(el('span', 'card-reading', word.language === 'en' ? 'ENGLISH' : word.reading || '暂无读音'), el('h3', '', word.surface), el('span', 'card-romaji', word.language === 'en' ? `原形 ${word.base}` : word.romaji || '—'));
  const meaning = el('p', 'card-meaning', word.meaning || '尚未添加释义，点击词语补充');
  const sentence = el('p', 'card-sentence', word.sentence || '在阅读中遇见这个词'); sentence.lang = word.language;
...
function showDetail(id) {
  ...
  container.append(el('div', 'eyebrow', 'A WORD TO REMEMBER'), el('p', 'detail-reading', word.reading || languageName(word.language)), el('h2', 'detail-title', word.surface), el('p', 'muted', `${word.romaji} · ${word.pos} · 原形 ${word.base}`));
  container.append(el('p', 'small muted', word.review?.lastReviewedAt ? `已复习 ${word.review.reviews} 次 · 下次 ${new Date(word.review.dueAt).toLocaleString('zh-CN')}` : '尚未复习 · 添加释义后可以开始背词'));
  ...
  const example = el('blockquote', 'detail-example', word.sentence || '还没有例句'); example.lang = word.language;
...
$('#word-language').onchange = () => { page = 1; render(); };
$('#previous').onclick = () => { page--; render(); }; $('#next').onclick = () => { page++; render(); };
$('#export').onclick = () => $('#export-dialog').showModal();
$('#export-json').onclick = () => { download('yomihibi-words.json', JSON.stringify({ app: 'yomihibi', version: 2, exportedAt: new Date().toISOString(), words }, null, 2), 'application/json'); $('#export-dialog').close(); };
...
$('#import-file').onchange = async event => {
  try {
    ...
    if (json.app !== 'yomihibi' || ![1, 2].includes(json.version)) throw new Error('请选择读日和导出的 JSON 备份文件');
